...
def main():
    ...
    for clinic in list_of_clinics:
        print(f"Clinic ID: {clinic.clinic_id} - Opening Hours: {clinic.opening_hour} to {clinic.closing_hour}")
        if clinic.scheduled_exams:
            print("Scheduled Exams:")
            for exam in clinic.scheduled_exams:
                print(f"  Exam Type: {exam['exam_type']} - Start: {exam['start_time']} - End: {exam['end_time']}")
        else:
